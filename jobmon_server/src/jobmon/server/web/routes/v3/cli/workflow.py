...
def get_shared_workflow_ids(workflow_args: Dict[str, Any]) -> List[int]:
    """Return a list of workflow ids that share the same args."""
    with SessionMaker() as session:
        with session.begin():
            where_clauses = []
            if "status" in workflow_args and workflow_args["status"]:
                where_clauses.append(Workflow.status.in_(workflow_args["status"]))
            if "id" in workflow_args and workflow_args["id"]:
                where_clauses.append(Workflow.id.in_(workflow_args["id"]))
            if "name" in workflow_args and workflow_args["name"]:
                where_clauses.append(Workflow.name == workflow_args["name"])
            if "owner" in workflow_args and workflow_args["owner"]:
                where_clauses.append(
                    Workflow.id.in_(
                        select(WorkflowRun.workflow_id).where(
                            WorkflowRun.user == workflow_args["owner"]
                        )
                    )
                )
            if "tool" in workflow_args and workflow_args["tool"]:
                where_clauses.append(
                    Workflow.tool_version_id.in_(
                        select(ToolVersion.id).where(
                            ToolVersion.tool_id.in_(
                                select(Tool.id).where(
                                    Tool.name == workflow_args["tool"]
                                )
                            )
                        )
                    )
                )
            if "wf_arg" in workflow_args and workflow_args["wf_arg"]:
                pass  # Handle workflow arguments if needed

            if where_clauses:
                sql = select(Workflow.id).where(*where_clauses)
