...
from jobmon.server.web.utils.json_utils import parse_node_ids, serialize_node_ids_for_api
...
async def get_downstream_tasks(request: Request) -> Any:
    ...
    with SessionMaker() as session:
        with session.begin():
            ...
            result = {
                row.id: [row.node_id, serialize_node_ids_for_api(row.downstream_node_ids)]
