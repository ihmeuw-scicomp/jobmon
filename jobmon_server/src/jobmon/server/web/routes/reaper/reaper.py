...
def fix_wf_inconsistency(workflow_id: int) -> Any:
    ...
    with session.begin():
        ...
        sql: Select[Tuple[Optional[int], Optional[str]]] = select(Workflow.id, Task.status).where(*query_filter)
        rows1: Sequence[Row[Tuple[Optional[int], Optional[str]]]] = session.execute(sql).all()
