...
def test_retries(
    ...
    if expected_exception:
        with pytest.raises(expected_exception, match=expected_msg):
            requester.send_request(f"{api_prefix}/time", {}, "get")
    else:
        requester.send_request(f"{api_prefix}/time", {}, "get")
...
def test_connection_retry(client_env, mocker, api_prefix):
    ...
    rc, resp = good_requester.send_request(f"{api_prefix}/time", {}, "get", tenacious=True)
...
def test_fail_fast(client_env, mocker, api_prefix):
    mock_content = mocker.patch(
    ...
    with pytest.raises(InvalidRequest) as exc:
        requester.send_request(f"{api_prefix}/no-route-should-fail", {}, "get")
...
def test_non_tenacious_request(client_env, mocker, api_prefix):
    mock_content = mocker.patch(
    ...
    with pytest.raises(InvalidResponse) as exc:
        requester.send_request(f"{api_prefix}/test_bad", {}, "get", tenacious=False)
...
def test_connection_timeout(client_env, mocker, tenacious, exception_type, api_prefix):
    ...
    if tenacious:
        with pytest.raises(exception_type, match="Exceeded HTTP request retry budget"):
            requester.send_request(f"{api_prefix}/some_route", {}, "get", tenacious=True)
        assert mock_send_request.call_count > 1
    else:
        with pytest.raises(exception_type):
            requester.send_request(f"{api_prefix}/some_route", {}, "get", tenacious=False)
        assert mock_send_request.call_count == 1
