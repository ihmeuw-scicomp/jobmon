...
export const getStatusColor = (status: string): string => {
    return taskStatusMeta[status?.toUpperCase()]?.color || taskStatusMeta.UNKNOWN.color;
};
...
export const getStatusSymbol = (status: string): string => {
    return taskStatusMeta[status?.toUpperCase()]?.symbol || taskStatusMeta.UNKNOWN.symbol;
}; 