...
export default function TaskTable({
    ...
    const columns = [
        ...
        columnHelper.accessor('task_status', {
            ...
            Cell: ({ row }) => {
                ...
                const statusData = workflow_status.find(
                    item => item.status === status
                );
                return (
                    <div>
                        <label className="label-middle">
                            <FaCircle className={statusData.circleClass} />{' '}
                        </label>
    ...
    if (!taskTemplateName) {
        return (
            <Typography sx={{ pt: 5 }}>
                Could not retrieve tasks for this task template.
            </Typography>
        );
    }
    ...
    if (tasks.isError) {
        return (
            <Typography sx={{ pt: 5 }}>
                Error loading tasks. Please refresh and try again.
            </Typography>
        );
    }
