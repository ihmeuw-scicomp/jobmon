...
import {
  formatDayjsDate,
  formatJobmonDate,
} from '@jobmon_gui/utils/DayTime.ts';
...
export default function ClusteredErrors({
  taskTemplateId,
  workflowId,
}: ClusteredErrorsProps) {
  ...
  const prefetchErrorDetails = async (
    nextErrorDetailIndex: boolean | ErrorSampleModalDetails
  ) => {
    /*
    ...
    await queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        ...
        const ti_id =
          nextErrorDetailIndex.sample_ids[nextErrorDetailIndex.sample_index];
        return axios
          .get(`${error_log_viz_url}${workflowId}/${taskTemplateId}/${ti_id}`, {
            ...jobmonAxiosConfig,
            data: null,
          })
          .then(r => {
  ...
  const columns = [
    ...
    columnHelper.accessor('first_error_time', {
      ...
      Cell: ({ renderedCellValue }) =>
        dayjs.isDayjs(renderedCellValue)
          ? formatDayjsDate(renderedCellValue)
          : renderedCellValue,
    }),
    ...
    columnHelper.accessor('actions', {
      ...
      Cell: ({ row }) => (
        <HtmlTooltip
          title={'Filter tasks table for tasks with this error'}
          arrow={true}
          placement={'right'}
        >
          <IconButton
  ...
  if (!taskTemplateId) {
    return (
      <Typography sx={{ pt: 5 }}>
        Select a task template from above to clustered errors
      </Typography>
    );
  }
  if (errors.isError) {
    return (
      <Typography>
        Unable to retrieve clustered errors. Please refresh and try again
      </Typography>
    );
  }
  ...
  const modalChildren = () => {
    ...
    const error =
      (errorDetails as ErrorDetails)?.data?.error_logs?.[0] || false;
    if (errorDetails.isError || !error) {
      return (
        <Typography>
          Failed to retrieve error details. Please refresh and try again
        </Typography>
      );
    }
        ...
        <Fade in={true}>
          <Box>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={3}>
                <Typography sx={labelStyles}>Workflow ID:</Typography>
              </Grid>
              <Grid item xs={9}>
                {error.workflow_id}
              </Grid>
              ...
              <Grid item xs={3}>
                <Typography sx={labelStyles}>WorkflowRun ID:</Typography>
              </Grid>
              <Grid item xs={9}>
              ...
              <Grid item xs={12}>
                <Typography sx={labelStyles}>Error Message:</Typography>
              </Grid>
              <Grid item xs={12}>
                <Button onClick={toggleLanguage}>
                  {language === 'python'
                    ? 'Switch to R Syntax Highlighting'
                    : 'Switch to Python Syntax Highlighting'}
                </Button>
                <ScrollableCodeBlock>
              ...
              <Grid item xs={12}>
                <Typography sx={labelStyles}>Task Instance stderr:</Typography>
              </Grid>
              <Grid item xs={12}>
                <ScrollableCodeBlock>
                  <SyntaxHighlighter language={language}>
