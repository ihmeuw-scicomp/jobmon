        ...
        try {
          ...
          requestedRuntime =
            !isNaN(reqRuntimeVal) && reqRuntimeVal > 0
              ...
              : undefined;
          requestedMemory =
            !isNaN(reqMemoryVal) && reqMemoryVal > 0 ? reqMemoryVal : undefined;
        } catch {
    ...
    () => ({
      ...
      meanRuntime:
        kpiRuntimes.length > 0
          ? kpiRuntimes.reduce((a, b) => a + b, 0) / kpiRuntimes.length
          : undefined,
      medianRuntime: calculateMedian(kpiRuntimes),
      minMemoryGiB:
        kpiMemoriesGiB.length > 0 ? Math.min(...kpiMemoriesGiB) : undefined,
      maxMemoryGiB:
        kpiMemoriesGiB.length > 0 ? Math.max(...kpiMemoriesGiB) : undefined,
      meanMemoryGiB:
        kpiMemoriesGiB.length > 0
          ? kpiMemoriesGiB.reduce((a, b) => a + b, 0) / kpiMemoriesGiB.length
          : undefined,
    ...
    <Box sx={{ bgcolor: 'grey.50', minHeight: '100vh', pb: 4 }}>
      ...
      >
        ...
        <Typography variant="body2" color="text.secondary">
          Analyze runtime and memory usage patterns across task instances. Use
          filters and brush selection to drill down into specific subsets of
          data.
        </Typography>
      </Box>
