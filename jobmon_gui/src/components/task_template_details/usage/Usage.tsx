...
// Types and queries
import { getWorkflowUsageQueryFn, WorkflowUsageQueryKey } from "@jobmon_gui/queries/GetWorkflowUsage.ts";
import { components } from '@jobmon_gui/types/apiSchema';
import { ScatterDataPoint, UsageProps, UsageKPIStats, ResourceEfficiencyMetrics } from '@jobmon_gui/types/Usage';
...
export default function Usage({ taskTemplateName, taskTemplateVersionId, workflowId }: UsageProps) {
    ...
    const queryKeyForWorkflowUsage: WorkflowUsageQueryKey = ["workflow_details", "usage", taskTemplateVersionId, workflowId];

    const usageInfo = useQuery<TaskTemplateResourceUsageResponse | undefined, Error, TaskTemplateResourceUsageResponse | undefined, WorkflowUsageQueryKey>({
    ...
    const filteredRequestedRuntimes = useMemo(() => {
        ...
        return rawTaskNodesFromApi
            .filter(d => {
                ...
                return selectedAttempts.has(String(d.attempt_number_of_instance || 1)) &&
                       selectedStatuses.has(String(d.status || 'UNKNOWN').toUpperCase()) &&
    ...
    }, [rawTaskNodesFromApi, selectedAttempts, selectedStatuses, selectedResourceClusters]);
    ...
    const medianRequestedRuntime = useMemo(() => calculateMedian(filteredRequestedRuntimes), [filteredRequestedRuntimes]);
    const medianRequestedMemoryGiB = useMemo(() => calculateMedian(filteredRequestedMemoriesGiB), [filteredRequestedMemoriesGiB]);
    ...
    const filteredScatterData = useMemo(() => {
        ...
        return rawTaskNodesFromApi
            ...
            .map((item): ScatterDataPoint | null => { // Ensure item.r and item.m are numbers for scatter plot
                ...
                try {
                    ...
                    requestedRuntime = !isNaN(reqRuntimeVal) && reqRuntimeVal > 0 ? reqRuntimeVal : undefined;
                    requestedMemory = !isNaN(reqMemoryVal) && reqMemoryVal > 0 ? reqMemoryVal : undefined;
                } catch (e) {
                    // Skip invalid JSON, leave as undefined
                }

                if (runtime !== null && runtime > 0 && memoryGiB !== null && memoryGiB > 0) {
                    return {
    ...
    const kpiRuntimes = useMemo(() => dataForKPICalculations.map(d => d.runtime), [dataForKPICalculations]);
    const kpiMemoriesGiB = useMemo(() => dataForKPICalculations.map(d => d.memory), [dataForKPICalculations]);

    const kpiStats: UsageKPIStats = useMemo(() => ({
        minRuntime: kpiRuntimes.length > 0 ? Math.min(...kpiRuntimes) : undefined,
        maxRuntime: kpiRuntimes.length > 0 ? Math.max(...kpiRuntimes) : undefined,
        meanRuntime: kpiRuntimes.length > 0 ? kpiRuntimes.reduce((a, b) => a + b, 0) / kpiRuntimes.length : undefined,
        medianRuntime: calculateMedian(kpiRuntimes),
        minMemoryGiB: kpiMemoriesGiB.length > 0 ? Math.min(...kpiMemoriesGiB) : undefined,
        maxMemoryGiB: kpiMemoriesGiB.length > 0 ? Math.max(...kpiMemoriesGiB) : undefined,
        meanMemoryGiB: kpiMemoriesGiB.length > 0 ? kpiMemoriesGiB.reduce((a, b) => a + b, 0) / kpiMemoriesGiB.length : undefined,
    ...
    }), [kpiRuntimes, kpiMemoriesGiB, medianRequestedRuntimeForKPI, medianRequestedMemoryGiBForKPI]);
    ...
    const resourceEfficiency: ResourceEfficiencyMetrics = useMemo(() => {
        ...
        // Filter rawTaskNodesFromApi to only include task IDs from dataForKPICalculations
        const taskIds = new Set(dataForKPICalculations.map(d => d.task_id));
        const relevantRawTaskNodes = rawTaskNodesFromApi.filter(item => taskIds.has(item.task_id));
        
        return calculateResourceEfficiency(dataForKPICalculations, relevantRawTaskNodes);
    }, [dataForKPICalculations, rawTaskNodesFromApi]);
    ...
    if (usageInfo.isLoading) {
        return (
            <div>
                <Grid container spacing={3} style={{ marginTop: '20px', paddingLeft: '15px', paddingRight: '15px', marginBottom: '30px' }} columns={{ xs: 12, sm: 12, md: 12 }}>
                    {[1, 2, 3].map(key => (
                ...
                <Grid container spacing={3} style={{ marginTop: '30px', paddingLeft: '15px', paddingRight: '15px' }} columns={{ xs: 12, sm: 12, md: 12 }}>
                     <Grid item xs={12}>
    ...
    return (
        <Box sx={{ bgcolor: 'grey.50', minHeight: '100vh', pb: 4 }}>
            ...
            <Box sx={{ 
                ...
                <Typography variant="h4" component="h1" fontWeight="bold" color="primary.main" sx={{ mb: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                    Analyze runtime and memory usage patterns across task instances. Use filters and brush selection to drill down into specific subsets of data.
                </Typography>
            </Box>
            ...
            <UsageKPICards 
                ...
                selectedDataCount={selectedData.length > 0 ? selectedData.length : undefined}
