...
export const calculateResourceEfficiency = (
    ...
    data.forEach(point => {
        ...
        if (
            point.requestedMemory &&
            point.requestedMemory > 0 &&
            point.requestedRuntime &&
            point.requestedRuntime > 0
        ) {
            ...
            const runtimeUtilization =
                (point.runtime / point.requestedRuntime) * 100;
    ...
    const runtimeStd = Math.sqrt(
        runtimes.reduce((sq, n) => sq + Math.pow(n - runtimeMean, 2), 0) /
            runtimes.length
    );
    const memoryMean = memories.reduce((a, b) => a + b, 0) / memories.length;
    const memoryStd = Math.sqrt(
        memories.reduce((sq, n) => sq + Math.pow(n - memoryMean, 2), 0) /
            memories.length
    );

    const outlierCount = data.filter(
        d =>
    ...
    const overallMemoryUtilization =
        totalRequestedMemory > 0
            ? (totalActualMemory / totalRequestedMemory) * 100
            : 0;
    const overallRuntimeUtilization =
        totalRequestedRuntime > 0
            ? (totalActualRuntime / totalRequestedRuntime) * 100
            : 0;
    ...
    return {
        ...
        overAllocatedMemory:
            validDataPoints > 0
                ? Math.round((overAllocatedMemory / validDataPoints) * 100)
                : 0,
        underAllocatedMemory:
            validDataPoints > 0
                ? Math.round((underAllocatedMemory / validDataPoints) * 100)
                : 0,
        overAllocatedRuntime:
            validDataPoints > 0
                ? Math.round((overAllocatedRuntime / validDataPoints) * 100)
                : 0,
        underAllocatedRuntime:
            validDataPoints > 0
                ? Math.round((underAllocatedRuntime / validDataPoints) * 100)
                : 0,
 ...
 */
export const createResourceClusterKey = (
    runtime: number,
    memory: number
): string => {
 ...
 */
export const createResourceClusterLabel = (
    runtime: number,
    memory: number,
    taskCount: number
): string => {
 ...
 */
export const extractResourceClusters = (
    rawData: TaskResourceVizItem[]
): ResourceCluster[] => {
    if (!rawData || rawData.length === 0) return [];

    const clusterMap = new Map<
        string,
        { runtime: number; memory: number; count: number }
    >();
 ...
 */
export const getResourceClusterKey = (
