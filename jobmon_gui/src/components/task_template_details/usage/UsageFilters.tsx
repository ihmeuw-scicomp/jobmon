        ...
        >
            ...
            <Grid container spacing={1.5} alignItems="flex-start">
                ...
                <Grid item xs={12} md={8}>
                    <Typography variant="caption" color="text.secondary" fontWeight="medium" sx={{ mb: 1, display: 'block', height: '1.2em', lineHeight: '1.2em' }}>
                        DATA FILTERS
                    </Typography>
                    <Box display="flex" gap={2} flexWrap="wrap">
                        <FormControl size="small" sx={{ minWidth: 200 }}>
                            <InputLabel id="attempt-filter-label">Attempts</InputLabel>
                            <Select
                                ...
                                onChange={(event) => {
                                    ...
                                    onSelectedAttemptsChange(new Set(typeof value === 'string' ? value.split(',') : value as string[]));
                                }}
                                ...
                                renderValue={(selected) => (
                                    <Box display="flex" flexWrap="wrap" gap={0.5}>
                                        ...
                                        {selected.length > 2 && (
                                            <Chip label={`+${selected.length - 2}`} size="small" variant="outlined" />
                                        )}
                                    </Box>
                                )}
                            ...
                            >
                                {availableAttempts.map((attempt) => (
                                    <MenuItem key={attempt} value={attempt} dense>
                                        <Checkbox checked={selectedAttempts.has(attempt)} size="small" />
                                        <ListItemText primary={`Attempt ${attempt}`} />
                                    </MenuItem>
                                ))}
                            </Select>
                        </FormControl>
                        ...
                        <FormControl size="small" sx={{ minWidth: 200 }}>
                            ...
                            <Select
                                ...
                                onChange={(event) => {
                                    const { target: { value } } = event;
                                    onSelectedStatusesChange(new Set(typeof value === 'string' ? value.split(',') : value as string[]));
                                }}
                                ...
                                renderValue={(selected) => (
                                    <Box display="flex" flexWrap="wrap" gap={0.5}>
                                        {selected.slice(0, 1).map((value) => (
                                            <Chip 
                                                key={value} 
                                                label={taskStatusMeta[value]?.label || value} 
                                                size="small"
                                                color={value === 'D' ? 'success' : 'error'}
                                                variant="outlined"
                                            />
                                        ))}
                                        {selected.length > 1 && (
                                            <Chip label={`+${selected.length - 1}`} size="small" variant="outlined" />
                                        )}
                                    </Box>
                                )}
                                MenuProps={{ PaperProps: { style: { maxHeight: 200 } } }}
                            >
                                {availableStatuses.map((statusKey) => (
                                    <MenuItem key={statusKey} value={statusKey} dense>
                                        <Checkbox checked={selectedStatuses.has(statusKey)} size="small" />
                                        <ListItemText primary={taskStatusMeta[statusKey]?.label || statusKey} />
                                    </MenuItem>
                                ))}
                            </Select>
                        </FormControl>
                        ...
                        <FormControl size="small" sx={{ minWidth: 250 }}>
                            <InputLabel id="resource-cluster-filter-label">Resource Clusters</InputLabel>
                            <Select
                                ...
                                onChange={(event) => {
                                    const { target: { value } } = event;
                                    onSelectedResourceClustersChange(new Set(typeof value === 'string' ? value.split(',') : value as string[]));
                                }}
                                input={<OutlinedInput label="Resource Clusters" />}
                                renderValue={(selected) => (
                                    <Box display="flex" flexWrap="wrap" gap={0.5}>
                                        {selected.slice(0, 1).map((value) => {
                                            const cluster = availableResourceClusters.find(c => c.id === value);
                                            return (
                                                <Chip 
                                                    key={value} 
                                                    label={cluster ? `${cluster.id} (${cluster.taskCount})` : value} 
                                                    size="small"
                                                    color="primary"
                                                    variant="outlined"
                                                />
                            ...
                            >
                                {availableResourceClusters.map((cluster) => (
                                    <MenuItem key={cluster.id} value={cluster.id} dense>
                                        <Checkbox checked={selectedResourceClusters.has(cluster.id)} size="small" />
                                        <ListItemText 
                        ...
                        <FormControlLabel
                            control={
                                <Switch 
                                    ...
                                    onChange={() => onShowResourceZonesChange(!showResourceZones)}
                                    color="primary"
                                    size="small"
                                />
                            }
