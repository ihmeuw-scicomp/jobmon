...
import { taskStatusMeta, getStatusColor } from '@jobmon_gui/constants/taskStatus';
import { calculateZoneBoundaries, calculateResourceZone } from './usageCalculations';
...
const RuntimeMemoryScatterPlot: React.FC<RuntimeMemoryScatterPlotProps> = ({ 
    ...
    const plotTraces = Object.keys(groupedData)
        ...
        .map((statusKey) => {
            ...
            const statusConfig = taskStatusMeta[statusKey] || taskStatusMeta.UNKNOWN;
            ...
            return {
                ...
                text: statusData.map(d => {
                    const pointStatusMeta = taskStatusMeta[String(d.status).toUpperCase()] || taskStatusMeta.UNKNOWN;
                    ...
                    if (d.requestedRuntime) {
                        const runtimeUtil = (d.runtime / d.requestedRuntime) * 100;
                ...
                customdata: statusData.map(d => ({ taskId: d.task_id, traceKey: statusKey })), // Add traceKey for identification
            };
        });
    ...
    if (showResourceZones && medianRequestedRuntime && medianRequestedMemory) {
        ...
        layout.annotations = [...layout.annotations, ...zoneBoundaries.annotations];
    }

    const handleClick = (event: any) => {
        if (event.points && event.points.length > 0) {
            const point = event.points[0];
            const customData = point.customdata as { taskId: number | string } | undefined;
    ...
    const handleSelected = (event: any) => {
        ...
        if (event && event.points && event.points.length > 0) {
            ...
            event.points.forEach((point: any) => {
                ...
                if (traceKey && groupedData[traceKey] && groupedData[traceKey][pointIndex]) {
...
const arePropsEqual = (prevProps: RuntimeMemoryScatterPlotProps, nextProps: RuntimeMemoryScatterPlotProps) => {
