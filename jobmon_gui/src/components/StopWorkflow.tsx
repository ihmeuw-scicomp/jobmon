...
export default function StopWorkflowButton({
  ...
  const user_name = user?.preferred_username
  ...
  const ModalChildren = () => {
          ...
          <Grid item xs={12}>
            ...
            >
              <Typography variant={'h6'}>
                Error stopping workflow. Please refresh the page and try again.
              </Typography>
            </Alert>
          </Grid>
      ...
      return (
        <Grid container spacing={2}>
          <Grid item xs={12}>
            ...
            >
              <Typography variant={'h6'}>
                Workflow stop request sent successfully.
              </Typography>
            </Alert>
          </Grid>
          <Grid item xs={12}></Grid>
    ...
    return (
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <Typography variant={'h6'}>
            Do you want to stop workflow {wf_id}? This action cannot be undone.
          </Typography>
        </Grid>
        <Grid item xs={12}></Grid>
        <Grid item xs={6}>
          <Button
            variant={'contained'}
            color={'error'}
            onClick={() => stopWorkflow.mutate(wf_id)}
          >
            Stop Workflow {wf_id}
          </Button>
        </Grid>
  ...
  return (
    <>
      <IconButton
        color="inherit"
        disabled={
          user_name != wfDetails.data.wfr_user ||
          stoppedStates.includes(wfDetails.data.wf_status) ||
          disabled
        }
        onClick={() => {
