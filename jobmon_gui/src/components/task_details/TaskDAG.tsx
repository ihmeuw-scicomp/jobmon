...
export default function TaskDAG({
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        const upstreamNodes: Node[] = upstreamTasks.map(
            (task: { id: string | number; name: string; status: string }) => ({
                ...
                position: { x: 0, y: 0 },
                style: {
                    backgroundColor: statusColorMap[task.status] || 'white',
                },
            })
        );
    ...
    const handleTaskStatusUpdate = async () => {
        ...
        try {
            ...
            setTaskUpdateMsg(
                'Error updating task statuses: ' + error.response.data.message
            );
        }
    };
                ...
                children={
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={9}>
                            <Link
                                to={{
                                    pathname: `/task_details/${selectedTaskId}`,
                                }}
                                key={selectedTaskId}
                        ...
                        <Grid item xs={9}>
                            ...
                            <IconButton
                                onClick={() =>
                                    setUpdateTaskCollepsOpen(prev => !prev)
                                }
                            >
                                {updateTaskCollepsOpen ? (
                                    <KeyboardArrowUpIcon />
                                ) : (
                        ...
                        <Grid item xs={3}>
                            <b>Task Status Date:</b>
                        </Grid>
                        <Grid item xs={9}>
                            {formatJobmonDate(
                                task_details?.data?.task_status_date
                            )}
                        </Grid>
                    </Grid>
                }
