...
export default function JobmonWFTable({ allData }) {
    ...
    const expandRow: ExpandRowProps<Data, never> = {
        renderer: row => {
            ...
            const num_attempts_avg = entry["num_attempts_avg"]
            const num_attempts_min = entry["num_attempts_min"]
            const num_attempts_max = entry["num_attempts_max"]
            ...
            return (
                <div>
                    <JobmonProgressBar
                        ...
                        fatal={fatal}
                        num_attempts_avg={num_attempts_avg}
                        num_attempts_min={num_attempts_min}
                        num_attempts_max={num_attempts_max}
                        maxc={maxc}
                        placement="top" />
                </div>
            )
            ...
            >
                {
                    props => (
                        <div>
                            ...
                            <ExportCSVButton {...props.csvProps} className="btn btn-custom mb-2 ml-2">Export CSV</ExportCSVButton>
