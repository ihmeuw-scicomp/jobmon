...
type WorkflowHeaderProps = {
  wf_id: number | string;
  task_template_info: { tt_version_id: string | number; name: string }[];
...
export default function WorkflowHeader({
  ...
  axios
    .get<WorkflowData>(get_workflow_concurrency_url(wf_id), {
      ...jobmonAxiosConfig,
      data: null,
    })
    .then(r => {
  ...
  const updateTaskTemplateConcurrency = useMutation({
    mutationFn: async ({
      ...
      return axios.put(
        set_task_template_concurrency_url(wf_id),
        {
  ...
  const handleUpdateStatus = (
    ...
    axios
        ...
        if (task_id_list.length > 10000 && recursive) {
          setStatusUpdateMsgDict(prevValues => ({
            ...
            [template.tt_version_id]:
              'Error: Too many tasks to update recursively. Please use non-recursive update.',
          }));
      ...
      .then(response => {
        ...
        if (
          response &&
          statusUpdateMsgDict[template.tt_version_id] !== 'Updating...'
        ) {
          setStatusUpdateMsgDict(prevValues => ({
            ...prevValues,
  ...
  useEffect(() => {
    ...
    task_template_info.map(template => {
      const url = get_task_template_concurrency_url(
        wf_id,
        template.tt_version_id
      );
      return axios.get<TaskTemplateData>(url, jobmonAxiosConfig).then(r => {
  ...
  const handleInputChange =
    (id: string | number) => (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      updateTaskTemplateConcurrency.mutate({
        task_template_version_id: id.toString(),
  ...
  if (wfDetails.isError) {
    return (
      <Typography>
        Error loading workflow details. Please refresh and try again.
      </Typography>
    );
  }
  ...
  const wfr_heartbeat_date = formatJobmonDate(
    wfDetails?.data?.wfr_heartbeat_date
  );
  ...
  function normalizeVersion(version: string): string {
    return version.replace(/\.dev/, '-dev').replace(/(\d+)rc(\d+)/, '$1-rc.$2');
  }
      ...
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        <span style={{ transform: 'translateY(-5px)', paddingLeft: '10px' }}>
          <HtmlTooltip
            ...
            sx={{ pl: 1 }}
          >
            <IconButton color="inherit" onClick={() => setShowWFInfo(true)}>
        ...
        <JobmonModal
          ...
          children={
            <Grid container spacing={2}>
              <Grid item xs={3} sx={gridHeaderStyles}>
                Workflow Status:
              </Grid>
              <Grid item xs={9}>
                {wf_status}
              ...
              <Grid item xs={3} sx={gridHeaderStyles}>
                Workflow Submitted Date:
              </Grid>
              <Grid item xs={9}>
                {wf_submitted_date}
              </Grid>
              <Grid item xs={3} sx={gridHeaderStyles}>
                WorkflowRun Heartbeat Date:
              </Grid>
              <Grid item xs={9}>
                {wfr_heartbeat_date}
              </Grid>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={9}>
                <Table>
                  <TableRow>
                    ...
                    <TableCell align="center">
                      <Tooltip
                        title={'Update the status of all tasks in the workflow'}
                      >
                  ...
                  <TableRow>
                    ...
                    <TableCell align="center">
                      <FormControl variant="outlined" size="small">
                        <InputLabel id="new-status-label">
                          New Status
                        </InputLabel>
                        <Select
                          ...
                          label="New Status"
                          onChange={e =>
                            handleUpdateStatusAll(e.target.value as string)
                          }
                          style={{ minWidth: 80 }}
                        >
              ...
              <Table>
                  ...
                  <TableCell align="center">
                    {recursive ? (
                      <Tooltip title="Update all tasks in the template recursively">
                        <span>
                          Update Task Status &nbsp; <AllInclusiveIcon />
                        </span>
                      </Tooltip>
                    ) : (
                ...
                {task_template_info?.map(template => (
                  <TableRow>
                    <TableCell align={'center'}>
                      <Typography sx={gridHeaderStyles}>
                        {template.name}
                      </Typography>
                    </TableCell>
                    <TableCell align="center">
                    ...
                    <TableCell align="center">
                      <FormControl variant="outlined" size="small">
                        ...
                        <Select
                          ...
                          label="New Status"
                          value={
                            statusSelectionDict[template.tt_version_id] || ''
                          }
                          onChange={e =>
                            handleUpdateStatus(
