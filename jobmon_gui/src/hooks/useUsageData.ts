...
import { ScatterDataPoint, UsageKPIStats, ResourceEfficiencyMetrics } from '@jobmon_gui/types/Usage';
import { calculateMedian, calculateResourceEfficiency } from '@jobmon_gui/components/task_template_details/usage/usageCalculations';
...
interface UseUsageDataReturn {
    ...
    handlePlotRestyle: (eventData: any) => void;
}
...
export const useUsageData = ({
    ...
    const previousTraceKeysRef = useRef<string>('');
    ...
    const filteredRequestedRuntimes = useMemo(() => {
        ...
        return rawTaskNodesFromApi
            .filter(d =>
                selectedAttempts.has(String(d.attempt_number_of_instance || 1)) &&
    ...
    const visibleRuntimes = useMemo(() => visibleScatterData.map(d => d.runtime), [visibleScatterData]);
    const visibleMemoriesGiB = useMemo(() => visibleScatterData.map(d => d.memory), [visibleScatterData]);

    const kpiStats: UsageKPIStats = useMemo(() => ({
        minRuntime: visibleRuntimes.length > 0 ? Math.min(...visibleRuntimes) : undefined,
        maxRuntime: visibleRuntimes.length > 0 ? Math.max(...visibleRuntimes) : undefined,
        meanRuntime: visibleRuntimes.length > 0 ? visibleRuntimes.reduce((a, b) => a + b, 0) / visibleRuntimes.length : undefined,
        medianRuntime: calculateMedian(visibleRuntimes),
        minMemoryGiB: visibleMemoriesGiB.length > 0 ? Math.min(...visibleMemoriesGiB) : undefined,
        maxMemoryGiB: visibleMemoriesGiB.length > 0 ? Math.max(...visibleMemoriesGiB) : undefined,
        meanMemoryGiB: visibleMemoriesGiB.length > 0 ? visibleMemoriesGiB.reduce((a, b) => a + b, 0) / visibleMemoriesGiB.length : undefined,
        medianMemoryGiB: calculateMedian(visibleMemoriesGiB),
        medianRequestedRuntime,
        medianRequestedMemoryGiB,
    }), [visibleRuntimes, visibleMemoriesGiB, medianRequestedRuntime, medianRequestedMemoryGiB]);
    ...
    const resourceEfficiency = useMemo(() => {
        ...
        const visibleTaskIds = new Set(visibleScatterData.map(d => d.task_id));
        const visibleRawTaskNodes = rawTaskNodesFromApi.filter(item => visibleTaskIds.has(item.task_id));
        
        return calculateResourceEfficiency(visibleScatterData, visibleRawTaskNodes);
    }, [visibleScatterData, rawTaskNodesFromApi]);

    // Handle Plotly restyle events (legend clicks) - using ref pattern from original
    const handlePlotRestyle = (eventData: any) => {
