...
export const useUsageData = ({
  ...
  const kpiStats: UsageKPIStats = useMemo(
    () => ({
      minRuntime:
        visibleRuntimes.length > 0 ? Math.min(...visibleRuntimes) : undefined,
      maxRuntime:
        visibleRuntimes.length > 0 ? Math.max(...visibleRuntimes) : undefined,
      meanRuntime:
        visibleRuntimes.length > 0
          ? visibleRuntimes.reduce((a, b) => a + b, 0) / visibleRuntimes.length
          : undefined,
