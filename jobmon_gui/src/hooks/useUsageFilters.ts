...
import { UsageFilters } from '@jobmon_gui/types/Usage';
import { components } from '@jobmon_gui/types/apiSchema';
import { extractResourceClusters, ResourceCluster } from '@jobmon_gui/components/task_template_details/usage/usageCalculations';
...
export const useUsageFilters = ({ rawTaskNodesFromApi }: UseUsageFiltersProps): UseUsageFiltersReturn => {
    // Filter state
    const [selectedAttempts, setSelectedAttempts] = useState<Set<string>>(new Set());
    const [selectedStatuses, setSelectedStatuses] = useState<Set<string>>(new Set());
    const [selectedResourceClusters, setSelectedResourceClusters] = useState<Set<string>>(new Set());

    // Calculate available filter options
    const availableAttempts = useMemo(() => {
        if (!rawTaskNodesFromApi) return [];
        const attempts = new Set(rawTaskNodesFromApi.map(d => String(d.attempt_number_of_instance || 1)));
        return Array.from(attempts).sort((a, b) => parseInt(a) - parseInt(b));
    }, [rawTaskNodesFromApi]);

    const availableStatuses = useMemo(() => {
        if (!rawTaskNodesFromApi) return [];
        const statuses = new Set(rawTaskNodesFromApi.map(d => String(d.status || 'UNKNOWN').toUpperCase()));
    ...
    useEffect(() => {
        setSelectedResourceClusters(new Set(availableResourceClusters.map(cluster => cluster.id)));
